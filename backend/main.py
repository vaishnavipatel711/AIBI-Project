...
# ─── Stock Data ───────────────────────────────────────────────────────
# INDIA_STOCKS: Indian companies traded as US-listed ADRs. Finnhub's free tier
# does not serve NSE/BSE-suffixed symbols (e.g. RELIANCE.NS), so these ADR
# tickers are the only Indian names with real, live-fetchable price data.
INDIA_STOCKS = {
    "INFY": "Infosys Ltd",
    "WIT": "Wipro Ltd",
    "IBN": "ICICI Bank Ltd",
...
# All Indian companies (NSE-style symbols) for name lookup / search breadth.
# Only the ones also in INDIA_STOCKS above have live price support; the rest
# are shown with clearly-labeled simulated data (see `data_source` field).
try:
    from companies_data import NSE_COMPANIES
except ImportError:
    NSE_COMPANIES = {}

# Unified name lookup used everywhere so a ticker never displays as itself.
ALL_COMPANIES = {**NSE_COMPANIES, **INDIA_STOCKS, **US_STOCKS}

# Tickers Finnhub can actually be queried for (rest fall back to simulated data).
LIVE_CAPABLE_TICKERS = set(INDIA_STOCKS.keys()) | set(US_STOCKS.keys())

# Users often type NSE/BSE-style suffixed symbols (e.g. INFY.NS, RELIANCE.BO).
# Finnhub's free tier and our lookup tables only know the bare symbol, so
# strip any recognized exchange suffix before resolving a ticker anywhere.
...
REGION_TICKERS = {
    ...
    "us": list(US_STOCKS.keys()),
    "global": list(INDIA_STOCKS.keys())[:6] + list(US_STOCKS.keys())[:6],
}
...
BASE_PRICES = {"INFY": 22.5, "WIT": 5.8, "IBN": 24.3, "HDB": 68.2, "TTM": 4.1,
        "RDY": 72.5, "VEDL": 16.8, "WNS": 62.3, "SIFY": 1.85, "MMYT": 78.4,
        "YTRA": 3.2, "AAPL": 185.5, "MSFT": 420.2, "GOOGL": 175.8, "AMZN": 185.3,
        "TSLA": 248.5, "META": 505.2, "NVDA": 875.3, "JPM": 195.8, "V": 275.4,
        "M": 18.2, "NFLX": 650.0, "DIS": 112.0}

def safe_finnhub_quote(ticker):
    """Fetch quote from Finnhub with fallback demo data.
    Adds a 'source' field: 'live' if from Finnhub, 'simulated' if generated
    locally (either Finnhub failed/rate-limited, or the symbol isn't a
    Finnhub-tradable ticker, e.g. an NSE-only name without a US ADR)."""
    if ticker in LIVE_CAPABLE_TICKERS:
        try:
            ...
            pass
    # Fallback demo data (deterministic-ish per ticker so repeated views look stable within a run)
    import random
    random.seed(hash(ticker) % (2**32) ^ int(time.time() // 3600))  # changes hourly, stable within the hour
    base = BASE_PRICES.get(ticker, 50.0)
    change = random.uniform(-3, 3)
    pc = base - change
    return {"c": base, "pc": pc, "h": base * 1.02, "l": base * 0.98, "o": pc, "t": int(time.time()), "source": "simulated"}
...
def market_data(region: str = "india"):
    stocks = REGION_TICKERS.get(region.lower(), REGION_TICKERS["india"])
    result = []
    for ticker in stocks:
        ...
        if quote and quote.get("c", 0) != 0:
            ...
            result.append({
                "ticker": ticker,
                "name": ALL_COMPANIES.get(ticker, ticker),
...
def get_stock(ticker: str):
    ...
    # Only serve tickers we actually recognize (live-capable ADR/US list, or
    # any name in the broader Indian company directory). This stops random
    # unrecognized strings from getting fabricated price data.
    if t not in ALL_COMPANIES and t not in LIVE_CAPABLE_TICKERS:
        raise HTTPException(status_code=404, detail=f"'{t}' is not a recognized ticker")
...
def search_symbol(query: str):
    q = normalize_ticker(query).lower()
    if not q:
        return []
    matches = [
        {"symbol": symbol, "description": name, "type": "Common Stock",
         "live": symbol in LIVE_CAPABLE_TICKERS}
        for symbol, name in ALL_COMPANIES.items()
        if q in symbol.lower() or q in name.lower()
    ]
    matches.sort(key=lambda m: (not m["live"], not m["description"].lower().startswith(q) and not m["symbol"].lower().startswith(q)))
    return matches[:15]
...
def company_news(ticker: str):
    data = safe_company_news(normalize_ticker(ticker))
    return [
        {
            "headline": a.get("headline"),
            "summary": a.get("summary"),
...
def get_intelligence(ticker: str, db: Session = Depends(get_db)):
    ...
    company_name = ALL_COMPANIES.get(t, t)
...
def get_score_history(ticker: str, db: Session = Depends(get_db)):
    ...
    records = db.query(ScoreHistory).filter(ScoreHistory.ticker == normalize_ticker(ticker)).order_by(ScoreHistory.id.desc()).limit(10).all()
    return [
        {
